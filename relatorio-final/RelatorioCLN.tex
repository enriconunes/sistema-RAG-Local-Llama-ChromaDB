...
semântica e geração de resposta decorre na própria máquina. Esta opção responde
à restrição central do enunciado: \emph{``que quaisquer recursos usados sejam
configurados localmente e que não haja chamadas a APIs durante a sua operação''}.
...
apenas do conhecimento paramétrico do modelo de linguagem, atacando diretamente
...
enviar dados para o exterior. Essa execução local é assegurada pelo
...
pesquisa por palavras-chave, usa-se \emph{dense retrieval}, popularizado pelo
\emph{Dense Passage Retrieval}~\cite{karpukhin2020dpr}, em que pergunta e
documentos são projetados no mesmo espaço vetorial e comparados por similaridade.
Para gerar estes vetores recorre-se à biblioteca \emph{Sentence-Transformers};
em particular, adota-se um modelo \textbf{multilingue} obtido por destilação de
conhecimento~\cite{reimers2020multilingual}, adequado a páginas em português.
...
\paragraph{Adaptação relevante face aos slides.}
As aulas T10 e T11 ilustram o \emph{pipeline} recorrendo, em alguns exemplos, a
serviços com API (por exemplo \texttt{ChatOpenAI} e \texttt{OpenAIEmbeddings}).
Como o enunciado proíbe chamadas a APIs em operação, substituíram-se esses
componentes pelos equivalentes \textbf{locais} ensinados na própria T11:
\texttt{ChatOpenAI}~$\rightarrow$~\textbf{ChatOllama} (Llama~3.2) e
\texttt{OpenAIEmbeddings}~$\rightarrow$~\textbf{Sentence-Transformers}
(\emph{HuggingFaceEmbeddings}). Mantém-se assim a arquitetura ensinada, mas
cumpre-se a restrição de funcionamento \emph{offline}.
...
\emph{notebook}. Os valores predefinidos seguem os usados na T11.
...
A escolha de um modelo de \emph{embeddings} \textbf{multilingue} (em vez do
\texttt{all-MiniLM-L6-v2} inglês dos slides) justifica-se por a fonte indexada
estar em português~\cite{reimers2020multilingual}.
...
Esta é a \textbf{fronteira online~$\rightarrow$~offline}: a partir daqui, nada
mais precisa de rede.
...
(\texttt{chunk\_size=1000}, \texttt{chunk\_overlap=100}), exatamente como na T11.
...
similaridade do cosseno equivalente ao produto interno. Este componente
substitui o \texttt{OpenAIEmbeddings} dos slides (que é uma API), preservando o
conceito de \emph{dense retrieval}~\cite{karpukhin2020dpr}.
...
Carrega-se o Llama~3.2~\cite{llama3} através do \texttt{ChatOllama}, tal como na
T11. Define-se \texttt{temperature=0} para tornar as respostas determinísticas e
menos propensas a invenção. Este componente substitui o \texttt{ChatOpenAI} dos
slides, eliminando a chamada à API.
...
aulas T09--T11, substituindo apenas os componentes baseados em API pelos seus
equivalentes locais.
...
com modelação de tópicos (LDA), conforme apresentado na T11.
